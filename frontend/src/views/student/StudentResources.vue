<template>
  ...
  <div ref="mountEl" class="external-react-page-host external-react-page-host--main" data-vue-wrapper="student-resources" />
</template>
...
function notifyUploaded(file: { id: number; knowledge_document_id?: number | null; parse_status?: string; knowledge_ingest_status?: string | null }) {
  window.dispatchEvent(new CustomEvent('student-resource-knowledge-updated', { detail: file }))
}

onMounted(() => {
  window.addEventListener('student-resource-upload-open', openUploadDialog)
  if (mountEl.value) {
    unmount = mountStudentExternalPage(mountEl.value, 'student-resources')
  }
})

onBeforeUnmount(() => {
  window.removeEventListener('student-resource-upload-open', openUploadDialog)
  unmount?.()
  unmount = null
})
</script>
