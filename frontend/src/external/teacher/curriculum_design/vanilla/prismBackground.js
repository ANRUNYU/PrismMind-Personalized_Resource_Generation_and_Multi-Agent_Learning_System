...
export function mountCurriculumPrismBackground(mount) {
  if (!mount) {
    return () => {};
  }
  ...
  shardGroup.position.set(0.14, -0.22, 0);
  shardGroup.scale.setScalar(window.innerWidth < 900 ? 0.9 : 1.02);
  ...
  function animate() {
    ...
    shardGroup.position.x = THREE.MathUtils.damp(shardGroup.position.x, 0.14 + pointer.x * 0.24 * motionFactor, 1.75, delta);
    shardGroup.position.y = THREE.MathUtils.damp(shardGroup.position.y, -0.22 + pointer.y * 0.08 * motionFactor, 1.75, delta);
    shardGroup.rotation.y = THREE.MathUtils.damp(shardGroup.rotation.y, pointer.x * 0.052 * motionFactor, 1.65, delta);
    shardGroup.rotation.x = THREE.MathUtils.damp(shardGroup.rotation.x, -pointer.y * 0.026 * motionFactor, 1.65, delta);
    ...
    shardGroup.children.forEach((shard) => {
      animateShard(shard, pointer, elapsed, delta, motionFactor);
    });
...
function animateShard(shard, pointer, elapsed, delta, motionFactor) {
  ...
  shard.rotation.x = THREE.MathUtils.damp(
    shard.rotation.x,
    data.baseRotation.x +
      elapsed * data.spinSpeed * data.spinAxis.x +
      Math.sin(elapsed * 0.24 + data.phase) * data.spinDrift * motionFactor +
      pointer.y * data.mouseInfluence * 0.82 * motionFactor,
    2.7,
    delta
  );
  shard.rotation.y = THREE.MathUtils.damp(
    shard.rotation.y,
    data.baseRotation.y +
      elapsed * data.spinSpeed * data.spinAxis.y +
      Math.cos(elapsed * 0.2 + data.phase) * data.spinDrift * 0.86 * motionFactor +
      pointer.x * data.mouseInfluence * 0.9 * motionFactor,
    2.7,
    delta
  );
  shard.rotation.z = THREE.MathUtils.damp(
    shard.rotation.z,
    data.baseRotation.z +
      elapsed * data.spinSpeed * data.spinAxis.z +
      Math.cos(elapsed * 0.22 + data.phase) * data.spinDrift * 0.95 * motionFactor,
    3,
    delta
  );
  ...
  data.bodyMaterial.emissiveIntensity = THREE.MathUtils.damp(
    data.bodyMaterial.emissiveIntensity,
    data.baseEmissiveIntensity * (0.76 + glowPulse * 0.62),
    2.8,
    delta
  );
  data.edgeMaterial.opacity = THREE.MathUtils.damp(
    data.edgeMaterial.opacity,
    data.baseEdgeOpacity * (0.78 + glowPulse * 0.45),
    2.8,
    delta
  );
}
...
function makeShardData(width) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const geometryType = isMain
      ? typeRoll < 0.58 ? "long" : "poly"
      : isMedium
        ? typeRoll < 0.38 ? "long" : "poly"
        : typeRoll < 0.44 ? "long" : "small";
    const x = THREE.MathUtils.lerp(2.02, 6.7, Math.pow(t, 0.6)) + THREE.MathUtils.lerp(-0.32, 0.34, seededRandom(index, 1));
    const y = isMain
        ? THREE.MathUtils.lerp(0.34, -2.12, seededRandom(index, 2)) + Math.sin(t * Math.PI * 1.4) * 0.28
      : isMedium
        ? THREE.MathUtils.lerp(0.72, -2.44, seededRandom(index, 2)) + Math.sin((t + 0.18) * Math.PI) * 0.2
        : THREE.MathUtils.lerp(0.84, -2.72, seededRandom(index, 2));
    const z = THREE.MathUtils.lerp(-1.7, 0.1, seededRandom(index, 3));
    const baseScale = {
      main: THREE.MathUtils.lerp(0.84, 1.28, seededRandom(index, 4)),
      medium: THREE.MathUtils.lerp(0.44, 0.82, seededRandom(index, 4)),
      small: THREE.MathUtils.lerp(0.2, 0.38, seededRandom(index, 4)),
      distant: THREE.MathUtils.lerp(0.2, 0.4, seededRandom(index, 4))
    }[role];
    const longShard = geometryType === "long";
    const polyShard = geometryType === "poly";
    const opacity = {
      main: THREE.MathUtils.lerp(0.54, 0.74, seededRandom(index, 11)),
      medium: THREE.MathUtils.lerp(0.3, 0.52, seededRandom(index, 11)),
      small: THREE.MathUtils.lerp(0.17, 0.34, seededRandom(index, 11)),
      distant: THREE.MathUtils.lerp(0.07, 0.15, seededRandom(index, 11))
    }[role];
    const bodyColor = isMain ? "#071338" : isMedium ? "#10264b" : role === "small" ? "#2b4361" : "#536f88";
    const motionScale = {
      main: 1,
      medium: 1.18,
      small: 1.46,
    ...
    return {
      ...
      rimIntensity: {
        ...
        small: 0.22,
        distant: 0.13
      }[role] * THREE.MathUtils.lerp(0.82, 1.24, seededRandom(index, 12)),
