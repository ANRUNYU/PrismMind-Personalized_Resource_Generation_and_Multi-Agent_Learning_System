// Copied from D:\Work Space\change\Teacher\training_program\src\components\PrismBackground.
// CSS is injected by ExternalTeacherTrainingPlans so the visual stays scoped inside the Shadow DOM.
// @ts-nocheck
...
function createIrregularPrismGeometry(type, seed) {
  ...
  if (type === "long") {
    ...
    const firstTip =
      tipAxis === "z"
        ? [tipSkew[0], tipSkew[1], -primaryLength]
        : [tipSkew[0], -primaryLength, tipSkew[1]];
    const secondTip =
      tipAxis === "z"
...
function makeShardData(width) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const x = rightCluster
      ? THREE.MathUtils.lerp(2.28, 6.68, Math.pow(t, 0.62)) + THREE.MathUtils.lerp(-0.34, 0.34, seededRandom(index, 1))
      : THREE.MathUtils.lerp(-5.7, -2.6, seededRandom(index, 1));
    const y = isMain
      ? THREE.MathUtils.lerp(-0.48, -2.34, seededRandom(index, 2)) + Math.sin(t * Math.PI * 1.4) * 0.28
      : isMedium
        ? THREE.MathUtils.lerp(0.38, -2.48, seededRandom(index, 2)) + Math.sin((t + 0.18) * Math.PI) * 0.18
        : rightCluster
          ? THREE.MathUtils.lerp(0.18, -2.7, seededRandom(index, 2))
          : THREE.MathUtils.lerp(-0.06, -1.34, seededRandom(index, 2));
    const z = rightCluster
      ? THREE.MathUtils.lerp(-1.72, 0.15, seededRandom(index, 3))
      : THREE.MathUtils.lerp(-2.5, -1.45, seededRandom(index, 3));
    const baseScale = {
      main: THREE.MathUtils.lerp(0.88, 1.32, seededRandom(index, 4)),
...
export default function PrismBackground() {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      shardGroup.position.y = THREE.MathUtils.damp(shardGroup.position.y, -0.28 + pointer.y * 0.075 * motionFactor, 1.75, delta);
  ...
  return <div className="resource-prism-background" ref={mountRef} aria-hidden="true" />;
}
