...
function getDisplayName(user: UserInfo | null) {
  if (!user) return '教师'
  return user.full_name || user.username || (user.email ? user.email.split('@')[0] : '教师')
}

function getRoleLabel(user: UserInfo | null) {
  if (user?.role === 'student') return '课程成员空间'
  if (user?.role === 'teacher') return '教师课程空间'
  ...
  return (
    <header className="top-nav" ref={navRef}>
      <button
        ...
        onClick={() => window.location.assign('/teacher/dashboard')}
      >
        <PrismMindLogo />
      ...
      <div className="top-nav-left">
        <button className="top-nav-button is-active" type="button" onClick={() => window.location.assign('/teacher/dashboard')}>
          首页
        </button>
