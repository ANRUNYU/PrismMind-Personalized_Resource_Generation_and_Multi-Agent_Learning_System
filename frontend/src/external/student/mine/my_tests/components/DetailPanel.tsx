...
function DetailPanel({
  ...
  if (!test) {
    return (
      <aside className="detail-panel test-detail-panel test-detail-panel--empty">
        <div className="detail-panel-motion" key="empty">
          <span className="detail-kicker">测验详情</span>
          <h2>等待选择测验</h2>
          <p>滚轮切换或点击堆叠测验后，右侧详情会独立展示。</p>
  ...
  const panelKey = `${test.id}-${detail?.status || test.status}-${Boolean(submitResult)}`

  return (
    <aside className="detail-panel test-detail-panel">
      <div className="detail-panel-motion" key={panelKey}>
        ...
        <small className="status-note">{status || '测验详情已准备就绪'}</small>

        {loading ? (
          <div className="test-loading">正在加载测验详情</div>
        ) : detail ? (
          <section className="test-answer-panel">
            ...
            {isSubmitted ? (
              <div className="test-submit-summary">
                <strong>本次测验得分：{submitResult?.score ?? detail.score ?? '-'}</strong>
          ...
          <div className="test-loading">请选择或生成一份测验。</div>
        )}
      </div>
    </aside>
  )
